...
-   **`meta.output`**: _(System-maintained)_ Actual output from successful test execution
    ...
    -   Included in your service listing during `usvc services upload`
...
-   `meta`: **[Optional]** Metadata object containing:
    ...
    -   `output`: _(System-maintained)_ Automatically populated by `usvc test run`
        ...
        -   Embedded into `meta.output` during `usvc services upload`
...
### 2. Upload Phase: `usvc services upload`
...
-   **Upload is flexible**: `usvc services upload` works even if `.out` files are missing (gracefully skips)
-   **User vs System fields**:
    -   `meta.requirements` and `meta.expect` are **user-maintained** (you write these)
    -   `meta.output` is **system-maintained** (auto-generated by `usvc data run-tests` and `usvc services upload`)
