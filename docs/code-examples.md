...
-   **`SERVICE_BASE_URL`**: Platform-provided gateway URL
...
| `SERVICE_BASE_URL` | Platform-provided | `https://api.unitysvc.com/v1` |
...
-   `SERVICE_BASE_URL` - Provider API endpoint
...
-   **`meta.output`**: _(System-maintained)_ Actual output from successful test execution
    ...
    -   Included in your service listing during `usvc data upload`
...
4. Sets environment variables (`UNITYSVC_API_KEY`, `SERVICE_BASE_URL`) from provider credentials
...
export SERVICE_BASE_URL="https://api.fireworks.ai/inference/v1"
python3 test.py
```
...
-   `meta`: **[Optional]** Metadata object containing:
    ...
    -   `output`: _(System-maintained)_ Automatically populated by `usvc test run`
        ...
        -   Embedded into `meta.output` during `usvc data upload`
...
Note: For API credentials (`UNITYSVC_API_KEY`, `SERVICE_BASE_URL`), use environment variables instead of template variables. These are set from `services_populator.envs` during testing.
...
usvc data run-tests --provider your-provider
...
### 2. Upload Phase: `usvc data upload`
...
-   **Upload is flexible**: `usvc data upload` works even if `.out` files are missing (gracefully skips)
-   **User vs System fields**:
    -   `meta.requirements` and `meta.expect` are **user-maintained** (you write these)
    -   `meta.output` is **system-maintained** (auto-generated by `usvc data run-tests` and `usvc data upload`)
...
**Problem:** `usvc data validate` reports Jinja2 syntax errors
